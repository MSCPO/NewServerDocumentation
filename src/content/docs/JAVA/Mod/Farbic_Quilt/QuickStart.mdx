...
运行 Spigot 系服务端需要 Java。或许你已经安装了 Java，但你可能需要升级 Java 版本。
...
> 如果你是第一次启动服务器，会遇到 [命令行提示 `You need to agree to the EULA in order to run the server. Go to eula.txt for more info.` 后自动退出了](/java/mod/farbic_quilt/faq/) 的问题。
